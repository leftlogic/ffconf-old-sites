(function() {
  ...
  const out = $$('#timer');
  const live = new Date('2018-11-08T00:00:00Z').getTime();

  function d(delta) {
    return (delta / (1000 * 60 * 60 * 24)).toFixed(7);
  }
  ...
  function span(s) {
    return `${s
      ...
      .map(
        _ =>
          `<span style="display: inline-block; width: 0.6em; text-align: center;">${_}</span>`
      )
      .join('')}`;
  }

  function update() {
    const delta = live - Date.now();
    const parts = d(delta).split('');
    parts.forEach((v, i) => {
      out.children[i].textContent = v;
    });
  ...
  function render() {
    ...
    if (!isNaN(delta)) {
      out.innerHTML = `${span(d(delta))}&nbsp;days`;
      requestAnimationFrame(update);
    }
  }
